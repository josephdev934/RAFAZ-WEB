...
const Services = () => {
  ...
  const renderServiceCard = (service, index) => (
    ...
    >
      ...
      >
        <a
          className="text-[#473BF0] flex items-center gap-2 text-sm sm:text-base"
    ...
    >
      <div className="text-center mb-10" data-aos="fade-up">
        ...
        <p className="text-gray-700 text-sm sm:text-base">
          We create websites that look stunning, load fast, and convert{" "}
          <br className="hidden sm:block" /> better.
        </p>
      </div>

      {/* First Row */}
      <div className="flex flex-wrap justify-center gap-8 lg:gap-10 mb-10">
        {servicesFirstRow.map(renderServiceCard)}
      </div>

      {/* Second Row */}
      <div className="flex flex-wrap justify-center gap-8 lg:gap-10">
        {servicesSecondRow.map(renderServiceCard)}
      </div>
    </section>
  );
};
